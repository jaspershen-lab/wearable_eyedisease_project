...
data_dir <- "3_data_analysis/3_prediction_modeling/1w_prediction/time_period_data_grouped/"  
output_dir <- "3_data_analysis/3_prediction_modeling/1w_prediction/time_period_data_grouped/results"
...
analyze_file_data <- function(file_path) {
  ...
  # 添加人口统计学和医疗史特征
  demographic_features <- c("age", "gender", "pre_vision", "season", "bmi")
  ...
  tryCatch({
    ...
    y <- model_data$vision_improvement
  ...
  # 使用选定的特征进行模型训练
  final_model_data <- model_data[, c(selected_features, "vision_improvement")]
  ...
  # 构建公式对象
  formula_obj <- as.formula(paste("vision_improvement ~", paste(selected_features, collapse = " + ")))
