...
# 设置工作路径（请根据需要调整）
input_dir <- "3_data_analysis/3_prediction_modeling/1m_prediction/daily_data_grouped"
output_dir <- "3_data_analysis/3_prediction_modeling/1m_prediction/daily_data_grouped/lmm_model_performance/surgery_group"

# 创建输出目录
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

# 获取所有分组的日期文件列表
d_files <- list.files(input_dir, pattern = "_D_Surg1\\.csv$", full.names = TRUE)
nod_files <- list.files(input_dir, pattern = "_NoD_Surg0\\.csv$", full.names = TRUE)

# 调试: 检查找到的文件
cat("找到的D_Surg1文件数量:", length(d_files), "\n")
cat("找到的NoD_Surg0文件数量:", length(nod_files), "\n")
...
analyze_group <- function(files, group_name, time_points_str) {
  ...
  for (i in 1:length(time_points_str)) {
    ...
    # 确定文件模式
    file_pattern <- if(startsWith(tp, "-")) {
      # 对于负数，我们使用转义的模式
      if(group_name == "D_Surg1") {
    ...
    # 构建完整模型
    full_model <- tryCatch({
      lm(vision_improvement_1m ~ mean_rhr_1 + mean_bo + steps_total + 
           age + gender + bmi + pre_vision + season + vision_improvement_1w, data = day_data)
    }, error = function(e) {
    ...
    # 构建单一预测变量模型
    hr_model <- lm(vision_improvement_1m ~ mean_rhr_1 + age + gender + bmi + pre_vision + season + vision_improvement_1w, 
                   data = day_data)
    bo_model <- lm(vision_improvement_1m ~ mean_bo + age + gender + bmi + pre_vision + season + vision_improvement_1w, 
                   data = day_data)
    steps_model <- lm(vision_improvement_1m ~ steps_total + age + gender + bmi + pre_vision + season + vision_improvement_1w, 
                      data = day_data)
    ...
    tryCatch({
      # 完整模型
      full_cv_r2 <- cv_r2(vision_improvement_1m ~ mean_rhr_1 + mean_bo + steps_total + 
                            age + gender + bmi + pre_vision + season + vision_improvement_1w)
      if(is.na(full_cv_r2)) {
      ...
      # HR模型
      hr_cv_r2 <- cv_r2(vision_improvement_1m ~ mean_rhr_1 + age + gender + bmi + pre_vision + season + vision_improvement_1w)
      if(is.na(hr_cv_r2)) {
      ...
      # BO模型
      bo_cv_r2 <- cv_r2(vision_improvement_1m ~ mean_bo + age + gender + bmi + pre_vision + season + vision_improvement_1w)
      if(is.na(bo_cv_r2)) {
      ...
      # 步数模型
      steps_cv_r2 <- cv_r2(vision_improvement_1m ~ steps_total + age + gender + bmi + pre_vision + season + vision_improvement_1w)
...
d_surg1_results <- analyze_group(d_files, "D_Surg1", time_points_str)
nod_surg0_results <- analyze_group(nod_files, "NoD_Surg0", time_points_str)

# 将结果保存到CSV文件
write.csv(d_surg1_results, file.path(output_dir, "model_performance_d_surg1_1m.csv"), row.names = FALSE)
write.csv(nod_surg0_results, file.path(output_dir, "model_performance_nod_surg0_1m.csv"), row.names = FALSE)

cat(paste0("D_Surg1组结果已保存到: ", file.path(output_dir, "model_performance_d_surg1_1m.csv"), "\n"))
cat(paste0("NoD_Surg0组结果已保存到: ", file.path(output_dir, "model_performance_nod_surg0_1m.csv"), "\n"))

# 为D_Surg1组创建绘图数据
plot_data_d <- data.frame(
  time_point = rep(d_surg1_results$time_point, 4),
  time_point_num = rep(d_surg1_results$time_point_num, 4),
  predictor = c(
    rep("All Predictors", nrow(d_surg1_results)),
    rep("HR", nrow(d_surg1_results)),
    rep("BO", nrow(d_surg1_results)),
    rep("Total Steps", nrow(d_surg1_results))
  ),
  r2 = c(
    d_surg1_results$all_r2,
    d_surg1_results$hr_only_r2,
    d_surg1_results$bo_only_r2,
    d_surg1_results$steps_only_r2
  )
)
...
p_d <- ggplot(plot_data_d, aes(x = time_point_num, y = r2, color = predictor, group = predictor)) +
  ...
  scale_color_manual(values = c("All Predictors" = "#513a52", 
                                ...
                                "Total Steps" = "#feefc4")) +
  labs(
    title = "Predictor Performance for Vision Improvement - D_Surg1 Group (1 Month)",
...
# 保存D_Surg1组图形
print(p_d)
ggsave(file.path(output_dir, "predictor_performance_d_surg1_1m.pdf"), p_d, width = 10, height = 6)
ggsave(file.path(output_dir, "predictor_performance_d_surg1_1m.png"), p_d, width = 10, height = 6, dpi = 300)

# 为NoD_Surg0组创建绘图数据
plot_data_nod <- data.frame(
  time_point = rep(nod_surg0_results$time_point, 4),
  time_point_num = rep(nod_surg0_results$time_point_num, 4),
  predictor = c(
    rep("All Predictors", nrow(nod_surg0_results)),
    rep("HR", nrow(nod_surg0_results)),
    rep("BO", nrow(nod_surg0_results)),
    rep("Total Steps", nrow(nod_surg0_results))
  ),
  r2 = c(
    nod_surg0_results$all_r2,
    nod_surg0_results$hr_only_r2,
    nod_surg0_results$bo_only_r2,
    nod_surg0_results$steps_only_r2
  )
)
...
p_nod <- ggplot(plot_data_nod, aes(x = time_point_num, y = r2, color = predictor, group = predictor)) +
  ...
  scale_color_manual(values = c("All Predictors" = "#513a52", 
                                ...
                                "Total Steps" = "#feefc4")) +
  labs(
    title = "Predictor Performance for Vision Improvement - NoD_Surg0 Group (1 Month)",
    x = "Days relative to surgery",
    y = "Cross-validated R²",
    color = "Predictors"
  ) +
...
# 保存NoD_Surg0组图形
print(p_nod)
ggsave(file.path(output_dir, "predictor_performance_nod_surg0_1m.pdf"), p_nod, width = 10, height = 6)
ggsave(file.path(output_dir, "predictor_performance_nod_surg0_1m.png"), p_nod, width = 10, height = 6, dpi = 300)

# 创建交互式版本 (如果安装了plotly)
if (requireNamespace("plotly", quietly = TRUE) && requireNamespace("htmlwidgets", quietly = TRUE)) {
  # D_Surg1组交互式图形
  p_d_interactive <- plotly::ggplotly(p_d)
  htmlwidgets::saveWidget(p_d_interactive, file.path(output_dir, "predictor_performance_d_surg1_1m_interactive.html"))
  
  # NoD_Surg0组交互式图形
  p_nod_interactive <- plotly::ggplotly(p_nod)
  htmlwidgets::saveWidget(p_nod_interactive, file.path(output_dir, "predictor_performance_nod_surg0_1m_interactive.html"))
