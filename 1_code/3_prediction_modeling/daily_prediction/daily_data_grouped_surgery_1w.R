...
# 创建两个空列表来存储每个组别的所有数据，用于后续合并
all_no_diabetes_surgery0_data <- list()
all_diabetes_surgery1_data <- list()
...
for(file_path in daily_files) {
  ...
  if(nrow(no_diabetes_surgery0) > 0) {
    ...
    # 将数据添加到列表中，用于后续合并
    # 添加day列以便在合并后识别数据来源
    no_diabetes_surgery0$day_str <- day_str
    all_no_diabetes_surgery0_data[[day_str]] <- no_diabetes_surgery0
  }
  ...
  if(nrow(diabetes_surgery1) > 0) {
    ...
    # 将数据添加到列表中，用于后续合并
    # 添加day列以便在合并后识别数据来源
    diabetes_surgery1$day_str <- day_str
    all_diabetes_surgery1_data[[day_str]] <- diabetes_surgery1
  }
}
...
# 1. 合并No Diabetes Surgery 0组数据
if(length(all_no_diabetes_surgery0_data) > 0) {
  combined_no_diabetes_surgery0 <- bind_rows(all_no_diabetes_surgery0_data)
  combined_file <- file.path(output_dir, "combined_NoD_Surg0.csv")
  write.csv(combined_no_diabetes_surgery0, combined_file, row.names = FALSE)
  cat("  已保存No Diabetes Surgery 0组合并数据到:", basename(combined_file), "\n")
  cat("  合并后样本量:", nrow(combined_no_diabetes_surgery0), "行\n")
} else {
  cat("  No Diabetes Surgery 0组没有数据可合并\n")
}

# 2. 合并Diabetes Surgery 1组数据
if(length(all_diabetes_surgery1_data) > 0) {
  combined_diabetes_surgery1 <- bind_rows(all_diabetes_surgery1_data)
  combined_file <- file.path(output_dir, "combined_D_Surg1.csv")
  write.csv(combined_diabetes_surgery1, combined_file, row.names = FALSE)
  cat("  已保存Diabetes Surgery 1组合并数据到:", basename(combined_file), "\n")
  cat("  合并后样本量:", nrow(combined_diabetes_surgery1), "行\n")
} else {
  cat("  Diabetes Surgery 1组没有数据可合并\n")
}
