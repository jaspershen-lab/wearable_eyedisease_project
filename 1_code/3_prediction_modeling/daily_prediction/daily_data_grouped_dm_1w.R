...
output_dir <- "3_data_analysis/3_prediction_modeling/1w_prediction/daily_data_grouped/dm_group"
...
# 显示分组信息的分布
cat("糖尿病状态分布:\n")
print(table(group_info$dm_status, dnn = c("糖尿病状态")))
...
for(file_path in daily_files) {
  ...
  if(nrow(no_diabetes) > 0) {
    out_file <- file.path(output_dir, paste0("day_", day_str, "_NoD.csv"))
    write.csv(no_diabetes, out_file, row.names = FALSE)
    cat("  已保存No Diabetes组数据到:", basename(out_file), "\n")
  }
  
  if(nrow(diabetes) > 0) {
    out_file <- file.path(output_dir, paste0("day_", day_str, "_D.csv"))
