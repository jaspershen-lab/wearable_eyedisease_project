# 术后1个月视力改善预测模型 - 按时间窗口分析
# 使用不同时间窗口的可穿戴设备数据预测术后1个月视力改善值
...
# 创建输出目录
output_dir <- "3_data_analysis/5_presurgery_analysis/vision_prediction/results_by_timewindow/1m"
...
# 检查combined_data是否包含所需视力变量
if(!"vision_improvement_1m" %in% colnames(combined_data)) {
  stop("vision_improvement_1m变量不在combined_data中，请检查数据")
}
...
prepare_timewindow_data <- function(data, time_window) {
  ...
  date_range <- switch(time_window,
                       "pre_7d_all" = c(-7, -1),      # 术前7天以前(假设最多考虑术前30天)
                       "pre_3d_7d" = c(-7, -4),        # 术前3-7天
                       "pre_3d" = c(-3, -1),           # 术前3天
                       "post_7d" = c(1, 7),            # 术后7天
  ...
  aggregated_data <- filtered_data %>%
    ...
    summarise(
      ...
      vision_improvement_1w = dplyr::first(vision_improvement_1w),  # 术后1周视力改善
      vision_improvement_1m = dplyr::first(vision_improvement_1m)   # 术后1个月视力改善（目标变量）
    )
...
# 目标变量 - 术后1个月视力改善
target_var <- "vision_improvement_1m"
...
vision_vars <- list(
  "pre_vision",           # 术前视力
  "vision_improvement_1w" # 术后1周视力改善
)
...
# 预测模型构建函数 - 适用于视力改善
build_prediction_models <- function(model_data, target_var, time_window = "unknown") {
  ...
  # 准备基本模型数据 - 移除含有NA的行
  base_vars <- c(target_var, unlist(wearable_vars), unlist(vision_vars), "age", "gender", "bmi", "dm_2")
  ...
  vision_formula_str <- paste(unlist(vision_vars), collapse = " + ")
  ...
  # 3. 仅使用视力相关变量的模型
  vision_only_formula <- as.formula(paste(
    target_var, "~", 
    vision_formula_str, " + ",
    "age + gender + bmi + dm_2"
  ))
  
  vision_only_model <- tryCatch({
    lm(vision_only_formula, data = base_data)
  }, error = function(e) {
    cat("构建视力变量模型错误:", e$message, "\n")
    return(NULL)
  })
  
  if(!is.null(vision_only_model)) {
    vision_only_summary <- summary(vision_only_model)
    vision_only_cv <- cv_model_performance(vision_only_formula, base_data)
    
    # 添加到性能数据框
    performance_df <- rbind(performance_df, data.frame(
      time_window = time_window,
      target_var = target_var,
      predictor = "Vision Variables Only",
      r_squared = vision_only_summary$r.squared,
      adj_r_squared = vision_only_summary$adj.r.squared,
      cv_r_squared = vision_only_cv$r_squared,
      cv_rmse = vision_only_cv$rmse,
      sample_size = nrow(base_data),
      predictor_coef = NA,
      predictor_p_value = NA,
      stringsAsFactors = FALSE
    ...
    # 打印视力变量模型信息
    cat("\n--- 仅使用视力相关变量 ---\n")
    cat("R²:", vision_only_summary$r.squared, "\n")
    cat("调整后R²:", vision_only_summary$adj.r.squared, "\n")
    cat("交叉验证R²:", vision_only_cv$r_squared, "\n")
    cat("交叉验证RMSE:", vision_only_cv$rmse, "\n")
  }
...
for(time_window in time_windows) {
  ...
  vision_performance <- tryCatch({
    build_prediction_models(current_window_data, target_var, time_window)
  }, error = function(e) {
...
# 保存到CSV文件
write.csv(all_vision_performance, file.path(output_dir, "vision_improvement_1m_prediction_by_timewindow.csv"), row.names = FALSE)
cat("\n所有模型性能结果已保存到:", file.path(output_dir, "vision_improvement_1m_prediction_by_timewindow.csv"), "\n")
...
create_trend_plots <- function(performance_data) {
  ...
  p <- ggplot(plot_data, aes(x = time_window, y = cv_r_squared, color = predictor, group = predictor)) +
    ...
    labs(
      title = "Prediction of Vision Improvement (1 Month)",
      x = "Time Window",
      y = "Cross-validated R²",
      color = "Predictors"
    ) +
  ...
  # 保存图表
  plot_filename <- "vision_improvement_1m_performance_trend.pdf"
  ggsave(file.path(output_dir, plot_filename), p, width = 10, height = 6)
  
  # 同时保存PNG格式
  plot_filename_png <- "vision_improvement_1m_performance_trend.png"
...
create_heatmap <- function(performance_data) {
  # 创建热图数据 - 排除"All Predictors"和"Vision Variables Only"
  heatmap_data <- performance_data %>%
    filter(!predictor %in% c("All Predictors", "Vision Variables Only")) %>%
    select(time_window, predictor, cv_r_squared) %>%
    # 确保时间窗口按照定义的顺序排序
    mutate(
  ...
  p_heatmap <- ggplot(heatmap_data, aes(x = time_window, y = predictor, fill = cv_r_squared)) +
    ...
    labs(
      title = "Predictive Power for Vision Improvement (1 Month)",
      x = "Time Window",
      y = "Predictor",
  ...
  # 保存热图
  heatmap_filename <- "vision_improvement_1m_heatmap.pdf"
  ggsave(file.path(output_dir, heatmap_filename), p_heatmap, width = 10, height = 6)
  
  # 同时保存PNG格式
  heatmap_filename_png <- "vision_improvement_1m_heatmap.png"
...
create_importance_plots <- function(performance_data) {
  # 筛选需要比较的预测变量数据（排除"All Predictors"和"Vision Variables Only"）
  var_data <- performance_data %>%
    filter(!predictor %in% c("All Predictors", "Vision Variables Only"))
  
  # 对每个预测变量，计算显著结果（p<0.05）的比例和平均R²
  ...
  p1 <- ggplot(significance_data, aes(x = reorder(predictor, significance_rate), y = significance_rate)) +
    ...
    labs(
      title = "Predictor significance for 1-month vision improvement",
  ...
  p2 <- ggplot(significance_data, aes(x = reorder(predictor, mean_rsquared), y = mean_rsquared)) +
    ...
    labs(
      title = "Average predictive power for 1-month vision improvement",
...
# 创建预测变量重要性图表
create_importance_plots(all_vision_performance)

#-------------------------------
# 12. 对比视力变量与可穿戴设备变量的预测能力
#-------------------------------
create_comparison_plot <- function(performance_data) {
  # 筛选出"Vision Variables Only"和"All Predictors"的数据
  comparison_data <- performance_data %>%
    filter(predictor %in% c("Vision Variables Only", "All Predictors")) %>%
    select(time_window, predictor, cv_r_squared) %>%
    # 确保时间窗口按照定义的顺序排序
    mutate(time_window = factor(time_window, levels = time_windows))
  ...
  color_values <- c(
    "All Predictors" = "#513a52",
    "Vision Variables Only" = "#d95f02"
  )
  
  # 创建对比折线图
  p <- ggplot(comparison_data, aes(x = time_window, y = cv_r_squared, color = predictor, group = predictor)) +
    geom_line(size = 1.2) +
    geom_point(size = 4) +
    scale_color_manual(
      values = color_values,
      labels = c(
        "All Predictors" = "All Variables (Wearable + Vision)",
        "Vision Variables Only" = "Vision Variables Only (pre_vision + 1w improvement)"
      )
    ) +
    labs(
      title = "Comparison of prediction models for 1-month vision improvement",
      x = "Time Window",
      y = "Cross-validated R²",
      color = "Model Type"
    ) +
  ...
  # 保存图表
  comparison_filename <- "vision_vs_all_comparison.pdf"
  ggsave(file.path(output_dir, comparison_filename), p, width = 10, height = 6)
  
  # 同时保存PNG格式
  comparison_filename_png <- "vision_vs_all_comparison.png"
  ggsave(file.path(output_dir, comparison_filename_png), p, width = 10, height = 6, dpi = 300)
  
  cat("已保存视力变量与全部变量的预测能力对比图\n")
  
  return(p)
}

# 创建视力变量与可穿戴设备变量预测能力对比图
comparison_plot <- create_comparison_plot(all_vision_performance)
