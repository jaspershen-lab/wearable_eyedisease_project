# 每日数据视力改善预测模型 (1个月)
# 使用每天的可穿戴设备数据以及早期视力指标预测术后1个月视力改善值
...
# 创建输出目录
output_dir <- "3_data_analysis/5_presurgery_analysis/vision_prediction/results_by_day/1m"
...
# 检查combined_data是否包含vision_improvement_1m
...
prepare_daily_data <- function(data, day_point) {
  ...
  processed_data <- filtered_data %>%
    select(
      ...
      vision_improvement_1m  # 术后1个月视力改善 (目标变量)
    )
...
# 预测模型构建函数 - 适用于视力改善
build_prediction_models <- function(model_data, target_var, additional_vars, day_point = "unknown") {
...
for(day_point in daily_timepoints) {
  ...
  vision_performance <- tryCatch({
    build_prediction_models(current_day_data, target_var, additional_vars, day_point)
  }, error = function(e) {
...
# 保存到CSV文件
write.csv(all_vision_performance, file.path(output_dir, "vision_improvement_1m_prediction_by_day.csv"), row.names = FALSE)
cat("\n所有模型性能结果已保存到:", file.path(output_dir, "vision_improvement_1m_prediction_by_day.csv"), "\n")
...
create_trend_plots <- function(performance_data) {
  ...
  # 保存图表
  plot_filename <- "vision_improvement_1m_daily_performance_trend.pdf"
  ggsave(file.path(output_dir, plot_filename), p, width = 10, height = 6)
  
  # 同时保存PNG格式
  plot_filename_png <- "vision_improvement_1m_daily_performance_trend.png"
...
create_heatmap <- function(performance_data) {
  ...
  # 保存热图
  heatmap_filename <- "vision_improvement_1m_daily_heatmap.pdf"
  ggsave(file.path(output_dir, heatmap_filename), p_heatmap, width = 10, height = 6)
  
  # 同时保存PNG格式
  heatmap_filename_png <- "vision_improvement_1m_daily_heatmap.png"
...
create_importance_plot <- function(performance_data) {
  ...
  significance_data <- var_data %>%
    ...
    summarise(
      total_days = n(),
      significant_days = sum(predictor_p_value < 0.05, na.rm = TRUE),
      significance_rate = significant_days / total_days * 100,
      mean_rsquared = mean(cv_r_squared, na.rm = TRUE),
      mean_coef = mean(predictor_coef, na.rm = TRUE)
    ) %>%
    arrange(desc(significance_rate))
  
  # 创建水平条形图
  p <- ggplot(significance_data, aes(x = reorder(predictor, significance_rate), y = significance_rate)) +
    ...
    labs(
      ...
      subtitle = "Percentage of days with statistically significant effect (p<0.05)",
      x = "",
      y = "Percentage of days with significant effect (%)"
    ) +
    coord_flip() +
    theme_bw() +
    theme(
  ...
  # 保存图表
  importance_filename <- "predictor_importance.pdf"
  ggsave(file.path(output_dir, importance_filename), p, width = 8, height = 6)
  
  # 同时保存PNG格式
  importance_filename_png <- "predictor_importance.png"
  ggsave(file.path(output_dir, importance_filename_png), p, width = 8, height = 6, dpi = 300)
