...
dir.create("3_data_analysis/5_presurgery_analysis/wearable_prior_after_surgery_compare", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/5_presurgery_analysis/wearable_prior_after_surgery_compare")
...
cat("组别分布:\n")
print(table(combined_data$group, useNA = "ifany"))
cat("时间段分布:\n")
...
# 设置组值
dr_group_val <- min(combined_data$group, na.rm = TRUE)
cataract_group_val <- max(combined_data$group, na.rm = TRUE)
...
# 执行每组的绘图
for (group_val in c(dr_group_val, cataract_group_val)) {
  group_label <- ifelse(group_val == dr_group_val, "DR Group", "Cataract Group")
  group_color <- ifelse(group_val == dr_group_val, "#3182bd", "#e6550d")
  
  cat("\n\n==========================================\n")
  cat(group_label, "的各指标随时间变化\n")
  cat("==========================================\n\n")
  
  # 过滤特定组的数据
  group_data <- combined_data %>% filter(group == group_val)
  ...
  for (var_info in variables) {
    ...
    # 创建图表
    plot_title <- paste(var_info$title, "-", group_label)
    ...
    if (!is.null(plot)) {
      ...
      ggsave(
        filename = paste0(gsub(" ", "_", tolower(var_info$title)), "_", 
                          tolower(gsub(" ", "_", group_label)), ".pdf"),
        plot = plot,
        width = 10,
        height = 7,
        dpi = 300
  ...
  for (group_val in c(dr_group_val, cataract_group_val)) {
    ...
    for (var_info in variables) {
      ...
      temp_stats <- temp_stats %>%
        left_join(
          p_values %>% select(time_period = group2, p.adj, p.signif),
          by = "time_period"
        )
    ...
    write_csv(
      stat_results,
      paste0("stat_tables/", tolower(gsub(" ", "_", group_label)), "_stats.csv")
    )
...
create_interaction_plot <- function(model_data, var_info, interaction_p, group_colors) {
  ...
  interaction_model <- lm(as.formula(paste(var_info$var, "~ group * time_period + age + gender + bmi")), 
                          data = model_data)
  
  # Get estimated marginal means
  emm_interaction <- emmeans(interaction_model, specs = ~ group | time_period)
  emm_df <- as.data.frame(emm_interaction)
  
  # Convert group to factor with  labels
  emm_df$group <- factor(emm_df$group, 
                         levels = c(dr_group_val, cataract_group_val),
                         labels = c("DR Group", "Cataract Group"))
  
  # Create interaction plot with  annotations
  interaction_plot <- ggplot(emm_df, aes(x = time_period, y = emmean, group = group, color = group)) +
    geom_line(size = 1.2) +
...
# Assuming you have already run your ANCOVA analyses and have the results

# For interaction plots
# For each variable in your variables list:
# 1. Filter data for the variable
# 2. Run interaction model
# 3. Create and save English plot
...
for (var_info in variables) {
  ...
  formula_str <- paste(var_info$var, "~ group * time_period + age + gender + bmi")
  ...
  interaction_p <- anova_results["group:time_period", "Pr(>F)"]
  ...
  # Save plot
  ggsave(
    filename = paste0("ancova_results", gsub(" ", "_", tolower(var_info$title)), "_interaction.pdf"),
    plot = plot,
    width = 10,
    height = 7,
    dpi = 300
