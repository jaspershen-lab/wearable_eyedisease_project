...
# 创建输出目录
output_dir <- "3_data_analysis/5_presurgery_analysis/octa_prediction/results_by_day/1w/DR_surgery"
...
# 加载基础数据 - 使用日期信息
combined_data <- read_csv("3_data_analysis/3_prediction_modeling/1w_prediction/daily_data_grouped/combined_D_Surg1.csv")
...
prepare_daily_data <- function(data, day_point) {
  # 筛选指定日期的数据
  filtered_data <- data %>%
    filter(as.numeric(day_str) == as.numeric(day_point))
...
build_prediction_models <- function(model_data, target_pairs, model_type = "unknown", day_point = "unknown") {
  ...
  for(pair in target_pairs) {
    ...
    # 准备基本模型数据 - 移除含有NA的行
    base_vars <- c(improvement_var, unlist(wearable_vars), "age", "gender", "bmi")
    base_data <- model_data %>%
      dplyr::select(all_of(base_vars)) %>%
      na.omit()
    
    if(nrow(base_data) < 8) {  # 确保有足够的样本
...
# 确保所有需要的数据都存在并且已经准备好
if(!exists("bloodflow_improvement") || !exists("thickness_improvement")) {
  cat("重新计算连续改善值...\n")
  bloodflow_improvement <- calculate_improvement(octa_bloodflow_features, "血流")
  thickness_improvement <- calculate_improvement(octa_thickness_features, "厚度")
}
...
build_binary_model <- function(model_data, target_pairs, model_type = "unknown", day_point = "unknown") {
  # 创建数据框来存储每个目标变量和预测器组合的模型性能指标
  performance_df <- data.frame(
    day_point = character(),
    target_var = character(),
    ...
    run_model_and_record <- function(formula_str, predictor_name, base_data_input) {
      ...
      tryCatch({
        if(!is.na(auc_value)) {
          ...
          cat("ROC曲线已保存到:", pdf_path, "和", png_path, "\n")
        }
      }, error = function(e) {
...
#-------------------------------
# 17. 对每个日期运行简化版二分类预测模型
#-------------------------------

cat("\n开始为每个日期构建简化版OCTA二分类改善预测模型...\n")
...
for(day_point in daily_timepoints) {
  ...
  binary_bloodflow_performance <- tryCatch({
    build_binary_model(model_data_binary_bloodflow, significant_binary_bloodflow_pairs, "bloodflow", day_point)
  }, error = function(e) {
  ...
  binary_thickness_performance <- tryCatch({
    build_binary_model(model_data_binary_thickness, significant_binary_thickness_pairs, "thickness", day_point)
  }, error = function(e) {
...
create_binary_trend_plots <- function(performance_data, data_type) {
  ...
  for(target in unique_targets) {
    ...
    # 保存图表
    plot_filename <- paste0("simple_binary_", data_type, "_", simple_target, "_daily_performance_trend.pdf")
    ggsave(file.path(output_dir, plot_filename), p, width = 10, height = 6)
    
    # 同时保存PNG格式
    plot_filename_png <- paste0("simple_binary_", data_type, "_", simple_target, "_daily_performance_trend.png")
    ggsave(file.path(output_dir, plot_filename_png), p, width = 10, height = 6, dpi = 300)
...
create_binary_heatmaps <- function(performance_data, data_type) {
  ...
  for(target in unique_targets) {
    ...
    # 保存热图
    heatmap_filename <- paste0("simple_binary_", data_type, "_", simple_target, "_daily_heatmap.pdf")
    ggsave(file.path(output_dir, heatmap_filename), p_heatmap, width = 10, height = 6)
    
    # 同时保存PNG格式
    heatmap_filename_png <- paste0("simple_binary_", data_type, "_", simple_target, "_daily_heatmap.png")
    ggsave(file.path(output_dir, heatmap_filename_png), p_heatmap, width = 10, height = 6, dpi = 300)
