...
# 创建输出目录
output_dir <- "3_data_analysis/5_presurgery_analysis/octa_prediction/results_by_timewindow/1m/no_prior7"
...
prepare_timewindow_data <- function(data, time_window) {
  ...
  date_range <- switch(time_window,
                       ...
                       "pre_3d" = c(-3, -1),           # 术前3天
                       "post_7d" = c(1, 7),            # 术后7天
                       "post_7d_30d" = c(8, 30),       # 术后7-30天
                       "post_day23_30" = c(23, 30),    # 术后23-30天
                       "post_day27_30" = c(27, 30),    # 术后27-30天
                       c(-30, 30)                       # 默认范围，应该不会用到
  )
...
#-------------------------------
# 13. 创建R²箱线图展示不同预测指标的表现
#-------------------------------

# 创建输出目录
r2_output_dir <- file.path(output_dir, "r2_boxplots")
dir.create(r2_output_dir, recursive = TRUE, showWarnings = FALSE)

# 将血流和厚度数据分开处理
all_bloodflow_performance_for_plots <- all_bloodflow_performance
all_thickness_performance_for_plots <- all_thickness_performance

# 创建R²箱线图的函数
create_r2_boxplots <- function(performance_data, data_type = "Combined") {
  # 提取所有唯一的目标变量
  unique_targets <- unique(performance_data$target_var)
  ...
  plot_data <- performance_data %>%
    select(target_var, predictor, cv_r_squared) %>%
    # 美化目标变量名称
    mutate(
      target_var_display = gsub("_improvement$", "", target_var),
      target_var_display = gsub("^(PA|VD|SVD|Thickness)_", "", target_var_display),
      target_var_display = gsub("_0_6", "", target_var_display)
    )
  
  # 计算每个预测变量的R²中位数
  r2_medians <- plot_data %>%
    group_by(predictor) %>%
    summarise(
      median_r2 = median(cv_r_squared, na.rm = TRUE),
      .groups = "drop"
    )
  ...
  plot_data <- plot_data %>%
    mutate(
      predictor_display = case_when(
        predictor == "All Predictors" ~ "All Predictors",
        predictor == "mean_rhr_1" ~ "Heart Rate",
        predictor == "mean_bo" ~ "Blood Oxygen",
        predictor == "steps_total" ~ "Total Steps",
        predictor == "total_sleep" ~ "Sleep Duration",
        TRUE ~ predictor
      )
    )
  
  r2_medians <- r2_medians %>%
    mutate(
      predictor_display = case_when(
        predictor == "All Predictors" ~ "All Predictors",
        predictor == "mean_rhr_1" ~ "Heart Rate",
  ...
  color_values <- c(
    ...
    "Blood Oxygen" = "#faaa93", 
    "Total Steps" = "#feefc4",
    "Sleep Duration" = "#7fb77e"
  )
  
  # 创建箱线图
  p1 <- ggplot(plot_data, aes(x = predictor_display, y = cv_r_squared, fill = predictor_display)) +
    geom_boxplot(alpha = 0.7, outlier.shape = 16, outlier.size = 2) +
    geom_text(data = r2_medians, 
              aes(x = predictor_display, y = median_r2, label = sprintf("%.3f", median_r2)),
              vjust = -0.8, size = 3.5, fontface = "bold") +
    scale_fill_manual(values = color_values) +
    labs(
      title = paste(data_type, "OCTA: R² Distribution by Predictor Variable"),
      subtitle = "Across all time windows and outcome measures",
      x = "Predictor",
      y = "Cross-validated R²",
      fill = "Predictor"
    ) +
  ...
  # 2. 按目标变量分组创建箱线图
  # 计算每个目标变量的R²中位数
  r2_target_medians <- plot_data %>%
    group_by(target_var_display) %>%
    summarise(
      median_r2 = median(cv_r_squared, na.rm = TRUE),
      .groups = "drop"
    )
  ...
  # 创建目标变量的箱线图
  p2 <- ggplot(plot_data, aes(x = target_var_display, y = cv_r_squared, fill = target_var_display)) +
    geom_boxplot(alpha = 0.7, outlier.shape = 16, outlier.size = 2) +
    geom_text(data = r2_target_medians, 
              aes(x = target_var_display, y = median_r2, label = sprintf("%.3f", median_r2)),
              vjust = -0.8, size = 3.5, fontface = "bold") +
    labs(
      title = paste(data_type, "OCTA: R² Distribution by Outcome Measure"),
      subtitle = "Across all time windows and predictors",
      x = "Outcome Measure",
      y = "Cross-validated R²",
  ...
  # 3. 按目标变量和预测变量分组创建箱线图
  # 计算每个目标变量和预测变量组合的R²中位数
  r2_combined_medians <- plot_data %>%
    group_by(target_var_display, predictor_display) %>%
    summarise(
      median_r2 = median(cv_r_squared, na.rm = TRUE),
      .groups = "drop"
    )
  ...
  # 创建组合箱线图
  p3 <- ggplot(plot_data, aes(x = predictor_display, y = cv_r_squared, fill = predictor_display)) +
    geom_boxplot(alpha = 0.7, outlier.shape = 16, outlier.size = 2) +
    geom_text(data = r2_combined_medians, 
              aes(x = predictor_display, y = median_r2, label = sprintf("%.3f", median_r2)),
              vjust = -0.8, size = 3, fontface = "bold") +
    scale_fill_manual(values = color_values) +
    facet_wrap(~ target_var_display, ncol = 2) +
    labs(
      title = paste(data_type, "OCTA: R² Distribution by Predictor and Outcome Measure"),
      subtitle = "Across all time windows",
      x = "Predictor",
      y = "Cross-validated R²",
      fill = "Predictor"
    ) +
  ...
  # 返回所有图表
  return(list(by_predictor = p1, by_target = p2, combined = p3))
}

# 创建和保存血流数据的R²箱线图
bloodflow_plots <- create_r2_boxplots(all_bloodflow_performance_for_plots, "Blood Flow")
ggsave(file.path(r2_output_dir, "bloodflow_r2_by_predictor.pdf"), bloodflow_plots$by_predictor, width = 10, height = 6)
ggsave(file.path(r2_output_dir, "bloodflow_r2_by_target.pdf"), bloodflow_plots$by_target, width = 8, height = 6)
ggsave(file.path(r2_output_dir, "bloodflow_r2_combined.pdf"), bloodflow_plots$combined, width = 12, height = 8)


# 创建和保存厚度数据的R²箱线图
thickness_plots <- create_r2_boxplots(all_thickness_performance_for_plots, "Thickness")
ggsave(file.path(r2_output_dir, "thickness_r2_by_predictor.pdf"), thickness_plots$by_predictor, width = 10, height = 6)
ggsave(file.path(r2_output_dir, "thickness_r2_by_target.pdf"), thickness_plots$by_target, width = 8, height = 6)
ggsave(file.path(r2_output_dir, "thickness_r2_combined.pdf"), thickness_plots$combined, width = 12, height = 8)


# 合并所有性能数据
all_performance_for_plots <- rbind(
  all_bloodflow_performance_for_plots,
  all_thickness_performance_for_plots
)

# 创建和保存所有数据的R²箱线图
all_plots <- create_r2_boxplots(all_performance_for_plots, "All OCTA")


# 打印摘要统计信息 - 每个预测变量的中位数R²
r2_summary <- all_performance_for_plots %>%
  group_by(predictor) %>%
  summarise(
    Median_R2 = median(cv_r_squared, na.rm = TRUE),
    Min_R2 = min(cv_r_squared, na.rm = TRUE),
    Max_R2 = max(cv_r_squared, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
ggsave(file.path(r2_output_dir, "all_r2_by_predictor.pdf"), all_plots$by_predictor, width = 10, height = 6)
ggsave(file.path(r2_output_dir, "all_r2_by_target.pdf"), all_plots$by_target, width = 10, height = 6)
ggsave(file.path(r2_output_dir, "all_r2_combined.pdf"), all_plots$combined, width = 14, height = 10)
