...
#########################
# Step 4: Create non-diabetes cataract vs diabetes PPV group information
#########################
...
#########################
dir.create("3_data_analysis/4_correlation_analysis/sleep/sleep_mixed_effects_models/nd_cat_vs_dm_ppv", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/4_correlation_analysis/sleep/sleep_mixed_effects_models/nd_cat_vs_dm_ppv")


#########################
# Step 7: Analyze each sleep metric by group
#########################

# Function to analyze and visualize a specific sleep metric
analyze_and_visualize_sleep <- function(metric_type) {
  # Merge sleep data with group information
  ...
  p_trend <- ggplot(avg_data, aes(x = day, y = mean_value, color = nd_cat_vs_dm_ppv, group = nd_cat_vs_dm_ppv)) +
    ...
    labs(
      title = paste0(display_name, " by group (No Diabetes Cataract vs Diabetes PPV)"),
  ...
  ggsave(paste0("nd_cat_vs_dm_ppv_", metric_type, "_time_trend.pdf"), p_trend, width = 10, height = 6, dpi = 300)
  
  # 2. Create box plot for group comparisons
  group_plot <- ggplot(long_data, aes(x = nd_cat_vs_dm_ppv, y = value, fill = nd_cat_vs_dm_ppv)) +
    # Add boxplot
  ...
  # Save the box plot
  ggsave(paste0("nd_cat_vs_dm_ppv_", metric_type, "_stats_comparison.pdf"), group_plot, width = 10, height = 6, dpi = 300)
  
  # 3. Create forest plot for pre-post differences
  group_contrasts <- extract_contrasts(lme_model, "nd_cat_vs_dm_ppv")
  ...
  group_forest_sig <- create_forest_plot_with_sig(
    ...
    "nd_cat_vs_dm_ppv", 
    surgery_colors,
    paste0(display_name, ": Preoperative and postoperative differences by group"),
    paste0(display_name, " (minutes)")
  )
  
  # Save the forest plot
  ggsave(paste0("nd_cat_vs_dm_ppv_", metric_type, "_forest_plot_period.pdf"), group_forest_sig, width = 10, height = 6, dpi = 300)
  
  # 4. Create between-group comparison forest plot
  group_comparison <- extract_group_comparison(lme_model, "nd_cat_vs_dm_ppv", "No_Diabetes_Cataract")
  
  group_forest <- create_comparison_forest_plot(
    group_comparison,
    surgery_colors,
    "No_Diabetes_Cataract",
    paste0("Diabetes PPV vs No Diabetes Cataract ", display_name, " Differences"),
    paste0(display_name, " (minutes)")
  )
  
  # Save the comparison forest plot
  ggsave(paste0("nd_cat_vs_dm_ppv_", metric_type, "_comparison_forest.pdf"), group_forest, width = 10, height = 8, dpi = 300)
...
# Analyze total sleep
total_sleep_results <- analyze_and_visualize_sleep("total_sleep")
print("Completed total sleep analysis and visualization")

# Analyze deep sleep
deep_sleep_results <- analyze_and_visualize_sleep("deep_sleep")
print("Completed deep sleep analysis and visualization")
...
create_summary_table <- function() {
  ...
  extract_model_stats <- function(model_name, model) {
    ...
    interaction_row <- which(rownames(anova_result) == "nd_cat_vs_dm_ppv:period")
    interaction_p <- anova_result[interaction_row, "p-value"]
    
    # Extract pre-post differences for each group
    group_contrasts <- extract_contrasts(model, "nd_cat_vs_dm_ppv")
    ...
    diabetes_ppv_row <- which(group_contrasts$nd_cat_vs_dm_ppv == "Diabetes_PPV")
    no_diabetes_cataract_row <- which(group_contrasts$nd_cat_vs_dm_ppv == "No_Diabetes_Cataract")
    ...
    data.frame(
      ...
      Diabetes_PPV_Pre_Post_Diff = sprintf("%.1f", group_contrasts$estimate[diabetes_ppv_row]),
      Diabetes_PPV_P = format_p(group_contrasts$p.value[diabetes_ppv_row]),
      No_Diabetes_Cataract_Pre_Post_Diff = sprintf("%.1f", group_contrasts$estimate[no_diabetes_cataract_row]),
      No_Diabetes_Cataract_P = format_p(group_contrasts$p.value[no_diabetes_cataract_row])
    )
  }
  ...
  # Save as CSV
  write.csv(result, "sleep_nd_cat_vs_dm_ppv_summary.csv", row.names = FALSE)
