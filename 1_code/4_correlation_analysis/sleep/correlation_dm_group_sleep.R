...
#########################
dir.create("3_data_analysis/4_correlation_analysis/sleep/sleep_mixed_effects_models/diabetes_group", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/4_correlation_analysis/sleep/sleep_mixed_effects_models/diabetes_group")

#########################
# Step 7: Analyze each sleep metric by diabetes group
#########################

# Function to analyze and visualize a specific sleep metric
analyze_and_visualize_sleep <- function(metric_type) {
  # Merge sleep data with diabetes group information
  ...
  dm_forest_sig <- create_forest_plot_with_sig(
    ...
    "dm_group", 
    diabetes_colors,
    paste0(display_name, ": Preoperative and postoperative differences by diabetes status"),
  ...
  dm_forest <- create_comparison_forest_plot(
    ...
    diabetes_colors,
    "No Diabetes",
    paste0("Diabetes vs Non-Diabetes ", display_name, " Differences"),
    paste0(display_name, " (minutes)")
  )
...
create_summary_table <- function() {
  ...
  # Save as CSV
  write.csv(result, "sleep_diabetes_summary.csv", row.names = FALSE)
