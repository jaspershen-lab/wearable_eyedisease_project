...
#########################
# Step 4: Create three original groups
#########################
...
create_comparison_forest_plot <- function(contrast_df, colors_map, reference_group,
  ...
  p <- ggplot(contrast_df, aes(x = estimate, y = comparison)) +
    ...
    geom_errorbarh(aes(xmin = lower.CL, xmax = upper.CL, color = comparison), height = 0.2) +
    # Add significance markers
    geom_text(aes(x = sig_position, label = significance), 
              hjust = 0, size = 5) +
    scale_color_manual(values = colors_map) +
...
#########################
dir.create("3_data_analysis/4_correlation_analysis/steps/steps_mixed_effects_models/original_groups", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/4_correlation_analysis/steps/steps_mixed_effects_models/original_groups")


#########################
# Step 7: Analyze each steps metric by group
#########################
...
analyze_and_visualize_steps <- function(metric_type) {
  ...
  p_trend <- ggplot(avg_data, aes(x = day, y = mean_value, color = orig_group, group = orig_group)) +
    ...
    labs(
      title = paste0("Steps (", metric_type, ") by original groups"),
  ...
  ggsave(paste0("orig_group_", metric_type, "_time_trend.pdf"), p_trend, width = 10, height = 6, dpi = 300)
  
  # 2. Create box plot for group comparisons
  group_plot <- ggplot(long_data, aes(x = orig_group, y = value, fill = orig_group)) +
    # Add boxplot
    geom_boxplot(alpha = 0.7, outlier.shape = NA) +
    # Add jittered points
    geom_jitter(aes(color = orig_group), width = 0.2, alpha = 0.5, size = 2) +
  ...
  # Save the box plot
  ggsave(paste0("orig_group_", metric_type, "_stats_comparison.pdf"), group_plot, width = 10, height = 6, dpi = 300)
  ...
  group_forest_sig <- create_forest_plot_with_sig(
    ...
    "orig_group", 
    original_colors,
    paste0("Steps (", metric_type, "): Preoperative and postoperative differences by group"),
    paste0("Steps (", metric_type, ")")
  )
  
  # Save the forest plot
  ggsave(paste0("orig_group_", metric_type, "_forest_plot_period.pdf"), group_forest_sig, width = 10, height = 6, dpi = 300)
  
  # 4. Create between-group comparison forest plot (using No_Diabetes_Surgery0 as reference)
  group_comparison <- extract_group_comparison(lme_model, "orig_group", "No_Diabetes_Surgery0")
  
  group_forest <- create_comparison_forest_plot(
    group_comparison,
    original_colors,
    "No_Diabetes_Surgery0",
    paste0("Group comparisons for Steps (", metric_type, ") vs No Diabetes Surgery0"),
    paste0("Steps (", metric_type, ")")
  )
  
  # Save the comparison forest plot
  ggsave(paste0("orig_group_", metric_type, "_comparison_forest.pdf"), group_forest, width = 10, height = 8, dpi = 300)
...
create_summary_table <- function() {
  ...
  extract_model_stats <- function(model_name, model) {
    ...
    no_diabetes_surgery0_row <- which(group_contrasts$orig_group == "No_Diabetes_Surgery0")
    diabetes_surgery0_row <- which(group_contrasts$orig_group == "Diabetes_Surgery0")
    diabetes_surgery1_row <- which(group_contrasts$orig_group == "Diabetes_Surgery1")
    
    # Create a row of summary data
    data.frame(
      Metric = model_name,
      Interaction_P = format_p(interaction_p),
      No_Diabetes_Surgery0_Pre_Post_Diff = sprintf("%.1f", group_contrasts$estimate[no_diabetes_surgery0_row]),
      No_Diabetes_Surgery0_P = format_p(group_contrasts$p.value[no_diabetes_surgery0_row]),
      Diabetes_Surgery0_Pre_Post_Diff = sprintf("%.1f", group_contrasts$estimate[diabetes_surgery0_row]),
      Diabetes_Surgery0_P = format_p(group_contrasts$p.value[diabetes_surgery0_row]),
      Diabetes_Surgery1_Pre_Post_Diff = sprintf("%.1f", group_contrasts$estimate[diabetes_surgery1_row]),
      Diabetes_Surgery1_P = format_p(group_contrasts$p.value[diabetes_surgery1_row])
    )
  }
  ...
  # Save as CSV
  write.csv(result, "steps_original_groups_summary.csv", row.names = FALSE)
