...
#########################
dir.create("3_data_analysis/4_correlation_analysis/steps/steps_mixed_effects_models/nd_cat_vs_dm_ppv", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/4_correlation_analysis/steps/steps_mixed_effects_models/nd_cat_vs_dm_ppv")
...
analyze_and_visualize_steps <- function(metric_type) {
  ...
  p_trend <- ggplot(avg_data, aes(x = day, y = mean_value, color = nd_cat_vs_dm_ppv, group = nd_cat_vs_dm_ppv)) +
    ...
    labs(
      title = paste0("Steps (", metric_type, ") by group (No Diabetes Cataract vs Diabetes PPV)"),
  ...
  group_forest <- create_comparison_forest_plot(
    ...
    paste0("Diabetes PPV vs No Diabetes Cataract Steps (", metric_type, ") Differences"),
    paste0("Steps (", metric_type, ")")
  )
...
create_summary_table <- function() {
  ...
  # Save as CSV
  write.csv(result, "steps_nd_cat_vs_dm_ppv_summary.csv", row.names = FALSE)
