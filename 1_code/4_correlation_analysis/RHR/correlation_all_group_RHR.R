...
# Create paired comparison forest plot
create_paired_comparison_forest_plot <- function(contrast_df, colors_map, 
                                                 title, metric_name = "RHR") {
  ...
  # Create readable contrast labels
  contrast_df$contrast_label <- paste(contrast_df$group1, "vs", contrast_df$group2)
...
#########################
dir.create("3_data_analysis/4_correlation_analysis/RHR/RHR_mixed_effects_models/three_groups", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/4_correlation_analysis/RHR/RHR_mixed_effects_models/three_groups")

#########################
# Step 7: Analyze each RHR metric
#########################

# Function to analyze and visualize a specific RHR metric with three groups
analyze_and_visualize_rhr_three_groups <- function(stat_type) {
  ...
  # Create display labels for better readability
  long_data <- long_data %>%
    mutate(group_label = case_when(
      orig_group == "No_Diabetes_Surgery0" ~ "Non-DM Cataract",
      orig_group == "Diabetes_Surgery0" ~ "DM Cataract",
      orig_group == "Diabetes_Surgery1" ~ "DM PPV",
      TRUE ~ orig_group
    ))
  ...
  # Map display labels to original colors
  display_colors <- c(
  ...
  p_trend <- ggplot(avg_data, aes(x = day, y = mean_value, color = group_label, group = group_label)) +
    ...
    labs(
      title = paste0("RHR (", stat_type, ") levels by group"),
  ...
  ggsave(paste0("three_groups_", stat_type, "_time_trend.pdf"), p_trend, width = 10, height = 6, dpi = 300)
  
  # 2. Create box plot for group comparisons
  group_plot <- ggplot(long_data, aes(x = group_label, y = value, fill = group_label)) +
    # Add boxplot
  ...
  # Save the box plot
  ggsave(paste0("three_groups_", stat_type, "_stats_comparison.pdf"), group_plot, width = 10, height = 6, dpi = 300)
  ...
  group_contrasts <- group_contrasts %>%
    mutate(group_display = case_when(
      group == "No_Diabetes_Surgery0" ~ "Non-DM Cataract",
      group == "Diabetes_Surgery0" ~ "DM Cataract",
      group == "Diabetes_Surgery1" ~ "DM PPV",
      TRUE ~ group
    ))
  
  group_forest_sig <- create_forest_plot_with_sig(
    group_contrasts %>% mutate(group = group_display), 
    "group", 
    display_colors,
    paste0("RHR (", stat_type, "): Preoperative and postoperative differences by group"),
    paste0("RHR (", stat_type, ")")
  )
  
  # Save the forest plot
  ggsave(paste0("three_groups_", stat_type, "_forest_plot_period.pdf"), group_forest_sig, width = 10, height = 6, dpi = 300)
  
  # 4. Create pairwise comparison forest plot
  paired_comparisons <- extract_paired_comparisons(lme_model, "orig_group")
  
  # Make group names more readable
  paired_comparisons <- paired_comparisons %>%
    mutate(
      group1_display = case_when(
        group1 == "No_Diabetes_Surgery0" ~ "Non-DM Cataract",
        group1 == "Diabetes_Surgery0" ~ "DM Cataract",
        group1 == "Diabetes_Surgery1" ~ "DM PPV",
        TRUE ~ group1
      ),
      group2_display = case_when(
        group2 == "No_Diabetes_Surgery0" ~ "Non-DM Cataract",
        group2 == "Diabetes_Surgery0" ~ "DM Cataract",
        group2 == "Diabetes_Surgery1" ~ "DM PPV",
        TRUE ~ group2
      ),
      contrast_label = paste(group1_display, "vs", group2_display)
    )
  
  paired_forest <- create_paired_comparison_forest_plot(
    paired_comparisons,
    display_colors,
    paste0("RHR (", stat_type, "): Pairwise Group Differences"),
    paste0("RHR (", stat_type, ")")
  )
  
  # Save the paired comparison forest plot
  ggsave(paste0("three_groups_", stat_type, "_paired_comparisons.pdf"), paired_forest, width = 12, height = 8, dpi = 300)
...
# Analyze mean RHR
mean_results <- analyze_and_visualize_rhr_three_groups("mean")
print("Completed mean RHR analysis and visualization")

# Analyze min RHR
min_results <- analyze_and_visualize_rhr_three_groups("min")
print("Completed min RHR analysis and visualization")

# Analyze max RHR
max_results <- analyze_and_visualize_rhr_three_groups("max")
print("Completed max RHR analysis and visualization")

# Analyze sd (standard deviation) of RHR
sd_results <- analyze_and_visualize_rhr_three_groups("sd")
