...
# Load RHR data instead of BO data
...
# Filter RHR data - changed from BO to RHR
...
#########################
dir.create("3_data_analysis/4_correlation_analysis/RHR/RHR_mixed_effects_models/diabetes_group", 
           recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/4_correlation_analysis/RHR/RHR_mixed_effects_models/diabetes_group")

#########################
# Step 7: Analyze each RHR metric
#########################

# Function to analyze and visualize a specific RHR metric
analyze_and_visualize_rhr <- function(stat_type) {
  # Merge RHR data with diabetes group information
  ...
  # Draw trend plot exactly like BO visualization
  ...
  ggsave(paste0("dm_group_", stat_type, "_time_trend.pdf"), p_trend, width = 10, height = 6, dpi = 300)
  
  # 2. Create box plot for group comparisons (matching BO code style)
  ...
  # Save the box plot
  ggsave(paste0("dm_group_", stat_type, "_stats_comparison.pdf"), dm_plot, width = 10, height = 6, dpi = 300)
  
  # 3. Create forest plot for pre-post differences (match BO visualization)
  ...
  dm_forest_sig <- create_forest_plot_with_sig(
    ...
    "dm_group", 
    diabetes_colors,
    paste0("RHR (", stat_type, "): Preoperative and postoperative differences by diabetes status"),
    paste0("RHR (", stat_type, ")")
  )
  
  # Save the forest plot
  ggsave(paste0("dm_group_", stat_type, "_forest_plot_period.pdf"), dm_forest_sig, width = 10, height = 6, dpi = 300)
  
  # 4. Create between-group comparison forest plot (match BO visualization)
  ...
  dm_forest <- create_comparison_forest_plot(
    ...
    diabetes_colors,
    "No Diabetes",
    paste0("Diabetes vs Non-Diabetes RHR (", stat_type, ") Differences"),
    paste0("RHR (", stat_type, ")")
  )
  
  # Save the comparison forest plot
  ggsave(paste0("dm_group_", stat_type, "_comparison_forest.pdf"), dm_forest, width = 10, height = 8, dpi = 300)
...
# Analyze mean RHR
mean_results <- analyze_and_visualize_rhr("mean")
print("Completed mean RHR analysis and visualization")

# Analyze min RHR
min_results <- analyze_and_visualize_rhr("min")
print("Completed min RHR analysis and visualization")

# Analyze max RHR
max_results <- analyze_and_visualize_rhr("max")
print("Completed max RHR analysis and visualization")

# Analyze sd (standard deviation) of RHR
sd_results <- analyze_and_visualize_rhr("sd")
print("Completed SD RHR analysis and visualization")

# Create a simple summary of results (avoiding complex table creation that caused errors)
print("Analysis complete. Visualizations saved in the output directory.")
