# -----------------------------------------------------
# 分析Group1和Group2各自内部的心率模式聚类对视力改善的影响
# 仅包含1周和1个月视力数据
# -----------------------------------------------------
library(tidyverse)
library(ggplot2)
...
# 1. 加载必要的数据
# -----------------------------------------------------
# 加载Group1和Group2的心率聚类结果
group1_clusters <- readRDS("3_data_analysis/6_clustering_modeling/kshape/time_series/1m/Group1_Surgery0_all_clusters.rds")
...
cluster1_assignments <- data.frame(
  ...
  hr_cluster = as.factor(cluster1_k2)
)
...
cluster2_assignments <- data.frame(
  ...
  hr_cluster = as.factor(cluster2_k2)
)
...
# 创建结果目录
output_dir_group1 <- "3_data_analysis/6_clustering_modeling/kshape/vision_analysis/1m/hr/Group1"
output_dir_group2 <- "3_data_analysis/6_clustering_modeling/kshape/vision_analysis/1m/hr/Group2"
...
analyze_vision_by_cluster <- function(data, vision_params, output_prefix, output_dir, group_name) {
  ...
  for (param in vision_params) {
    ...
    # 提取均值
    mean_c1 <- cluster_means$mean_value[cluster_means$hr_cluster == 1]
    mean_c2 <- cluster_means$mean_value[cluster_means$hr_cluster == 2]
    ...
    p <- ggstatsplot::ggbetweenstats(
      ...
      title = paste0(group_name, ": ", param_name, " 在不同心率模式组间的差异"),
...
analyze_vision_binary <- function(data, vision_params, output_prefix, output_dir, group_name) {
  ...
  for (param in vision_params) {
    ...
    for (method_name in names(binary_methods)) {
      ...
      # 创建列联表
      cont_table <- table(data$hr_cluster, data[[binary_var_name]])
      ...
      tryCatch({
        ...
        # 创建条形图
        cross_tab <- data %>%
          group_by(hr_cluster, !!sym(binary_var_name)) %>%
        ...
        p <- ggplot(cross_tab, aes(x = hr_cluster, y = percentage, fill = improvement)) +
          geom_bar(stat = "identity", position = "dodge") +
          geom_text(aes(label = sprintf("%.1f%%", percentage)), 
                    position = position_dodge(width = 0.9), vjust = -0.5) +
          labs(
            title = paste0(group_name, ": ", param_name, " - 不同心率聚类组中视力改善比例"),
  ...
  if (nrow(binary_results) > 0) {
    ...
    forestplot(
      ...
      title = paste(group_name, "- 心率聚类对视力改善的影响")
    )
...
create_combined_timepoint_plot <- function(data, group_name, output_dir) {
  ...
  # 创建分面图
  p <- ggplot(long_data, aes(x = hr_cluster, y = improvement, fill = timepoint)) +
    ...
    labs(
      title = paste(group_name, "- 不同心率模式聚类组的视力改善趋势"),
      x = "心率聚类组",
      y = "视力改善",
      fill = "时间点",
      color = "时间点"
...
create_html_report <- function(vision_analysis, vision_binary_analysis, membership_corr, output_dir, group_name) {
  ...
  # 创建HTML报告
  sink(file.path(output_dir, "heart_rate_cluster_vision_analysis_report.html"))
  
  cat('<!DOCTYPE html>
<html>
<head>
  <title>', group_name, ' - 心率模式聚类与视力改善关系分析报告</title>
  <style>
...
<body>
  <h1>', group_name, ' - 心率模式聚类与视力改善关系分析报告</h1>
  
  <div class="summary">
    <p>本报告分析了', group_name, '患者的不同心率恢复模式聚类组之间视力改善值的差异。分析包括连续变量分析和基于改善与否的二分类变量分析。</p>
  </div>
  ...
  cat('<p>本分析探讨了', group_name, '患者中不同心率恢复模式与视力改善之间的关系。总结如下：</p>')
  
  if(total_sig > 0) {
    cat('<p>心率聚类组间存在显著的视力改善差异，包括：</p>')
    cat('<ul>')
    if(nrow(vision_sig) > 0) {
    ...
    if(nrow(sig_corr) > 0) {
      ...
      for(i in 1:nrow(sig_corr)) {
        ...
        cat('<li>心率聚类成员度与', sig_corr$Parameter[i], '视力改善呈', direction, 
    ...
    cat('<p>这些结果表明', group_name, '患者的心率恢复模式与视力改善存在显著关联。建议进一步研究这些关联的临床意义。</p>')
  } else {
    cat('<p>在', group_name, '患者中，未发现心率聚类组间存在显著的视力改善差异。这可能表明：</p>')
    cat('<ul>')
    cat('<li>心率恢复模式与视力改善可能没有直接关系</li>')
  ...
  cat('<ol>')
  if(total_sig > 0) {
    cat('<li>临床实践中可考虑将心率恢复模式作为评估', group_name, '患者视力改善潜力的参考指标。</li>')
    cat('<li>建议进一步研究心率变异性与视力改善的生理机制。</li>')
    cat('<li>考虑将心率模式整合到预测模型中，以更准确预测患者术后视力改善情况。</li>')
  } else {
    cat('<li>对于', group_name, '患者，心率模式可能不是预测视力改善的有效指标。</li>')
    cat('<li>建议探索其他可能影响视力改善的因素。</li>')
  }
  cat('<li>设计更大规模的前瞻性研究，进一步验证心率模式与视力改善的关联性。</li>')
  cat('</ol>')
  ...
  cat("HTML报告已生成：", file.path(output_dir, "heart_rate_cluster_vision_analysis_report.html"), "\n")
}
...
# 保存处理后的数据集，以便进一步分析
saveRDS(vision_with_clusters_group1, file.path(output_dir_group1, "vision_with_hr_clusters.rds"))
saveRDS(vision_with_clusters_group2, file.path(output_dir_group2, "vision_with_hr_clusters.rds"))
