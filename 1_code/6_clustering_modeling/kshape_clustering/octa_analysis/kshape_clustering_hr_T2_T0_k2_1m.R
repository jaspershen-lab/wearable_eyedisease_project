...
cluster_assignments <- data.frame(
  ...
  hr_cluster = as.factor(cluster_k2)
)
...
# dir.create("3_data_analysis/6_clustering_modeling/kshape/octa_analysis/1m/T2_T0", recursive = TRUE, showWarnings = FALSE)
# setwd("3_data_analysis/6_clustering_modeling/kshape/octa_analysis/1m/T2_T0")

# 创建结果目录
output_dir <- "3_data_analysis/6_clustering_modeling/kshape/octa_analysis/1m/T2_T0"
...
bloodflow_analysis <- analyze_octa_by_cluster(
  ...
  "diabetes_bloodflow_by_hr_cluster"
)
...
thickness_analysis <- analyze_octa_by_cluster(
  ...
  "diabetes_thickness_by_hr_cluster"
)
...
analyze_octa_binary <- function(data, param_cols, output_prefix) {
  ...
  for (param in param_cols) {
    ...
    p <- ggplot(cross_tab, aes(x = hr_cluster, y = percentage, fill = improvement)) +
      ...
      labs(
        title = paste0(param_name, " - 不同心率聚类组中高/低改善比例"),
...
create_html_report <- function() {
  ...
  # 创建HTML报告
  sink(file.path(output_dir, "heart_rate_cluster_octa_analysis_report.html"))
  
  cat('<!DOCTYPE html>
<html>
<head>
  <title>心率模式聚类与OCTA改善关系分析报告</title>
  <style>
...
<body>
  <h1>心率模式聚类与OCTA改善关系分析报告</h1>
  
  <div class="summary">
    <p>本报告分析了糖尿病患者(Group2)的不同心率恢复模式聚类组之间OCTA参数改善值的差异。分析包括连续变量分析和基于中位数的二分类变量分析。</p>
  </div>
  ...
  if(nrow(bloodflow_binary_sig) > 0) {
    ...
    for(i in 1:nrow(bloodflow_binary_sig)) {
      ...
      cat('<td>', format(bloodflow_binary_sig$Median_Cutoff[i], digits=3), '</td>')
  ...
  if(nrow(thickness_binary_sig) > 0) {
    ...
    for(i in 1:nrow(thickness_binary_sig)) {
      ...
      cat('<td>', format(thickness_binary_sig$Median_Cutoff[i], digits=3), '</td>')
  ...
  if(total_sig > 0) {
    ...
    cat('<p>这些结果表明不同的心率恢复模式可能与术后OCTA参数的改善情况相关。建议进一步研究这些关联的临床意义。</p>')
  } else {
  ...
  cat("HTML报告已生成：", file.path(output_dir, "heart_rate_cluster_octa_analysis_report.html"), "\n")
}
