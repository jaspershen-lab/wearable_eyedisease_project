...
cluster_assignments <- data.frame(
  ...
  steps_cluster = as.factor(cluster_k3)
)

# 打印集群分布，查看每个集群的样本数
print(table(cluster_assignments$steps_cluster))
...
# 创建结果目录
# 可以根据使用的数据集选择适当的输出目录
# output_dir <- "3_data_analysis/6_clustering_modeling/octa_steps_cluster_analysis_k3"
output_dir <- "3_data_analysis/6_clustering_modeling/kshape/octa_analysis/1m/steps/T2_T0_k3"
  ...
  for (param in param_cols) {
    ...
    if (nrow(group1_comparisons) > 0) {
      for (i in 1:nrow(group1_comparisons)) {
        ...
        # 获取相关组的均值和标准差
        if (!is.na(comp_row$group1) && comp_row$group1 == "1") {
          group1_stats <- cluster_means[cluster_means$steps_cluster == "1", ]
          group2_stats <- cluster_means[cluster_means$steps_cluster == comp_row$group2, ]
        } else if (!is.na(comp_row$group2) && comp_row$group2 == "1") {
          group1_stats <- cluster_means[cluster_means$steps_cluster == "1", ]
          group2_stats <- cluster_means[cluster_means$steps_cluster == comp_row$group1, ]
        } else {
          # 处理无法确定比较的情况 - 跳过计算效应量
          next
        }
...
bloodflow_analysis <- analyze_octa_by_cluster_k3(
  ...
  "diabetes_bloodflow_by_steps_cluster_k3"
)
...
thickness_analysis <- analyze_octa_by_cluster_k3(
  ...
  "diabetes_thickness_by_steps_cluster_k3"
)
...
analyze_octa_binary_multinomial_k3 <- function(data, param_cols, output_prefix) {
  ...
  for (param in param_cols) {
    ...
    for (compare_group in setdiff(levels(data$steps_cluster), ref_group)) {
      # 创建仅包含参考组和比较组的子集
      subset_data <- data %>% 
        filter(steps_cluster %in% c(ref_group, compare_group))
      
      # 创建列联表
      contingency_table <- table(subset_data$steps_cluster, subset_data[[binary_var_name]])
      ...
      tryCatch({
        ...
        # 创建堆叠条形图
        p <- ggplot(plot_data, aes(x = steps_cluster, y = percentage, fill = improvement_status)) +
...
create_html_report <- function() {
  ...
  # 创建HTML报告
  sink(file.path(output_dir, "steps_cluster_k3_octa_analysis_report.html"))
  
  cat('<!DOCTYPE html>
<html>
<head>
  <title>步数模式聚类(K=3)与OCTA改善关系分析报告</title>
  <style>
...
<body>
  <h1>步数模式聚类(K=3)与OCTA改善关系分析报告</h1>
  
  <div class="summary">
    <p>本报告分析了糖尿病患者(Group2)的不同步数恢复模式聚类组(K=3)之间OCTA参数改善值的差异。分析包括连续变量分析和基于中位数的二分类变量分析，以组1为参照组，分析组2和组3对比组1的差异。</p>
  </div>
  ...
  # 二分类变量分析结果
  cat('<h2>2. 二分类变量分析结果 (基于改善>0的标准)</h2>')
  
  # 血流参数二分类结果 - 组2 vs 组1
  cat('<h3>2.1 血流参数 - 组2 vs 组1</h3>')
  
  if(nrow(bloodflow_binary_2vs1) > 0) {
    cat('<p>显著的血流参数Fisher检验结果 (组2 vs 组1):</p>')
    cat('<table>')
    cat('<tr><th>参数</th><th>P值</th><th>校正后P值</th></tr>')
    ...
    cat('<p class="non-significant">没有发现显著的血流参数Fisher检验结果 (组2 vs 组1)。</p>')
  }
  ...
  if(nrow(bloodflow_binary_3vs1) > 0) {
    cat('<p>显著的血流参数Fisher检验结果 (组3 vs 组1):</p>')
    cat('<table>')
    cat('<tr><th>参数</th><th>P值</th><th>校正后P值</th></tr>')
    ...
    cat('<p class="non-significant">没有发现显著的血流参数Fisher检验结果 (组3 vs 组1)。</p>')
  }
  ...
  if(nrow(thickness_binary_2vs1) > 0) {
    cat('<p>显著的厚度参数Fisher检验结果 (组2 vs 组1):</p>')
    cat('<table>')
    cat('<tr><th>参数</th><th>P值</th><th>校正后P值</th></tr>')
    ...
    cat('<p class="non-significant">没有发现显著的厚度参数Fisher检验结果 (组2 vs 组1)。</p>')
  }
  ...
  if(nrow(thickness_binary_3vs1) > 0) {
    cat('<p>显著的厚度参数Fisher检验结果 (组3 vs 组1):</p>')
    cat('<table>')
    cat('<tr><th>参数</th><th>P值</th><th>校正后P值</th></tr>')
    ...
    cat('<p class="non-significant">没有发现显著的厚度参数Fisher检验结果 (组3 vs 组1)。</p>')
  }
  ...
  if(total_sig > 0) {
    ...
    if(nrow(bloodflow_binary_2vs1) > 0) {
      cat('<li>血流参数二分类分析 (组2 vs 组1)：发现', nrow(bloodflow_binary_2vs1), '个显著差异</li>')
    }
    
    if(nrow(bloodflow_binary_3vs1) > 0) {
      cat('<li>血流参数二分类分析 (组3 vs 组1)：发现', nrow(bloodflow_binary_3vs1), '个显著差异</li>')
    }
    
    if(nrow(thickness_binary_2vs1) > 0) {
      cat('<li>厚度参数二分类分析 (组2 vs 组1)：发现', nrow(thickness_binary_2vs1), '个显著差异</li>')
    }
    
    if(nrow(thickness_binary_3vs1) > 0) {
      cat('<li>厚度参数二分类分析 (组3 vs 组1)：发现', nrow(thickness_binary_3vs1), '个显著差异</li>')
    }
    
    cat('</ul>')
    
    cat('<p>这些结果表明不同的步数恢复模式可能与术后OCTA参数的改善情况相关。建议进一步研究这些关联的临床意义，并探索不同步数模式组的临床特征差异。</p>')
  } else {
  ...
  cat("HTML报告已生成：", file.path(output_dir, "steps_cluster_k3_octa_analysis_report.html"), "\n")
}
