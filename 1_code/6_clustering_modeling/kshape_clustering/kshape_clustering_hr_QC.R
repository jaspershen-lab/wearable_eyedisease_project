...
# 添加数据质控
# 数据质控函数 - 基于文献描述
quality_control_data <- function(hr_trajectory_data, group_name) {
  # 1. 检查每个受试者的数据量
  subject_data_count <- hr_trajectory_data %>%
    group_by(subject_id) %>%
    summarize(
      data_points = n(),
    ...
    mutate(
      passed_qc = (unique_days >= 15) & has_periop_data  # 至少有30天的数据且在围手术期有数据
    )
    ...
    excluded_subjects <- subject_data_count %>% 
      ...
      mutate(
        reason = case_when(
          ...
          unique_days < 30 ~ paste0("Insufficient data (", unique_days, " days)"),
          TRUE ~ "Unknown reason"
        )
      )
  ...
  # 过滤通过质控的受试者数据
  hr_clean <- hr_trajectory_data %>%
    filter(subject_id %in% (subject_data_count %>% filter(passed_qc) %>% pull(subject_id)))
  
  cat("Original data points:", nrow(hr_trajectory_data), "\n")
  cat("Clean data points:", nrow(hr_clean), "(", round(nrow(hr_clean)/nrow(hr_trajectory_data)*100, 1), "%)\n")
...
group1_qc <- quality_control_data(group1_hr_data, "Group1_Surgery0")
group2_qc <- quality_control_data(group2_hr_data, "Group2_Diabetes")

# 使用通过质控的数据
group1_hr_data_clean <- group1_qc$clean_data
group2_hr_data_clean <- group2_qc$clean_data

# 创建结果保存目录并设置工作目录（只设置一次）
output_dir <- "3_data_analysis/6_clustering_modeling/kshape/time_series/1m"
...
saveRDS(group1_hr_data, "group1_hr_data_original.rds")
saveRDS(group2_hr_data, "group2_hr_data_original.rds")
saveRDS(group1_hr_data_clean, "group1_hr_data_clean.rds")
saveRDS(group2_hr_data_clean, "group2_hr_data_clean.rds")
...
group1_clusters <- perform_clustering(group1_hr_data_clean, "Group1_Surgery0")
group2_clusters <- perform_clustering(group2_hr_data_clean, "Group2_Diabetes")

# Analyze the clusters for each group (k=2 and k=3)
group1_analysis_k2 <- analyze_clusters(group1_hr_data_clean, group1_clusters, "Group1_Surgery0", 2)
group1_analysis_k3 <- analyze_clusters(group1_hr_data_clean, group1_clusters, "Group1_Surgery0", 3)

group2_analysis_k2 <- analyze_clusters(group2_hr_data_clean, group2_clusters, "Group2_Diabetes", 2)
group2_analysis_k3 <- analyze_clusters(group2_hr_data_clean, group2_clusters, "Group2_Diabetes", 3)
...
cat("- Subjects were required to have at least 30 days of data\n")
...
cat("Original subjects:", length(group2_subjects), "\n")
...
cat("- All subjects were required to have at least 30 days of data\n")
