...
# ================== 新增：全局缺失值分析函数 ==================
analyze_missing_data_global <- function(data, metrics, time_windows) {
  ...
  # 为每个时间窗口分析缺失情况
  window_completeness <- data.frame()
  ...
  for(window_name in names(time_windows)) {
    ...
    cat(sprintf("\n=== %s 时间窗口 (days %s) ===\n", 
                toupper(window_name), paste(range(window_days), collapse = " to ")))
    ...
    # 计算每个患者在该窗口的数据完整性
    window_data <- data %>% dplyr::select(subject_id, all_of(window_cols))
    
    completeness <- window_data %>%
      mutate(
        total_possible = length(window_cols),
        available_count = rowSums(!is.na(dplyr::select(., -subject_id))),
        completeness_rate = available_count / total_possible,
        is_complete = completeness_rate == 1.0,
        has_majority = available_count >= floor(total_possible / 2)
      )
    ...
    summary_stats <- completeness %>%
      summarise(
        window = window_name,
        total_patients = n(),
        complete_patients = sum(is_complete),
        majority_data_patients = sum(has_majority),
        avg_completeness = mean(completeness_rate),
        min_completeness = min(completeness_rate),
        max_completeness = max(completeness_rate)
      )
    
    window_completeness <- rbind(window_completeness, summary_stats)
    
    cat(sprintf("  完全完整数据: %d/%d (%.1f%%)\n", 
                summary_stats$complete_patients, 
                summary_stats$total_patients,
                summary_stats$complete_patients/summary_stats$total_patients*100))
    cat(sprintf("  至少一半数据: %d/%d (%.1f%%)\n", 
                summary_stats$majority_data_patients, 
                summary_stats$total_patients,
                summary_stats$majority_data_patients/summary_stats$total_patients*100))
    cat(sprintf("  平均完整度: %.1f%%\n", summary_stats$avg_completeness*100))
    
    # 显示数据不完整的患者
    incomplete_patients <- completeness %>%
      filter(!is_complete) %>%
      dplyr::select(subject_id, available_count, total_possible, completeness_rate)
    
    if(nrow(incomplete_patients) > 0) {
      cat("  数据不完整的患者:\n")
      for(i in 1:nrow(incomplete_patients)) {
        cat(sprintf("    %s: %d/%d (%.1f%%)\n", 
                    incomplete_patients$subject_id[i],
                    incomplete_patients$available_count[i],
                    incomplete_patients$total_possible[i],
                    incomplete_patients$completeness_rate[i]*100))
      }
    }
  }
  
  cat("\n========== 各时间窗口完整性汇总 ==========\n")
  print(window_completeness %>% 
          dplyr::select(window, total_patients, complete_patients, avg_completeness) %>%
          mutate(avg_completeness = round(avg_completeness*100, 1)))
  
  return(window_completeness)
}
...
calculate_window_membership_with_max <- function(data, window_info, metrics) {
  ...
  #   }
  # }
  # ================== 修改：根据时间窗口采用不同的数据完整性策略 ==================
  
  cat(sprintf("处理前患者数: %d\n", nrow(processed_data)))
  
  # 根据时间窗口制定不同的策略
  if(window_name %in% c("baseline", "acute_recovery", "early_recovery")) {
    # 前3个时间窗口：要求完全完整的数据
    cat(sprintf("窗口 %s：采用严格策略（要求100%%完整数据）\n", window_name))
    
    complete_patients <- processed_data %>%
      filter(rowSums(is.na(dplyr::select(., -subject_id))) == 0)
    
    strategy_used <- "complete_cases_only"
    
  } else if(window_name %in% c("mid_recovery", "late_recovery")) {
    # 后2个时间窗口：更严格的处理
    cat(sprintf("窗口 %s：采用灵活策略（但仍要求完整数据用于聚类）\n", window_name))
    
    # 🔧 修改：直接使用完整数据，不做复杂的80%筛选
    complete_patients <- processed_data %>%
      filter(rowSums(is.na(dplyr::select(., -subject_id))) == 0)
    
    # 如果完整数据太少，给出警告但继续
    original_count <- nrow(processed_data)
    final_count <- nrow(complete_patients)
    
    cat(sprintf("  原始患者数: %d\n", original_count))
    cat(sprintf("  完整数据患者数: %d\n", final_count))
    cat(sprintf("  数据完整率: %.1f%%\n", final_count/original_count*100))
    
    # 显示被排除的患者
    excluded_patients <- processed_data %>%
      filter(rowSums(is.na(dplyr::select(., -subject_id))) > 0)
    
    if(nrow(excluded_patients) > 0) {
      cat("  被排除的患者（有缺失值）:\n")
      for(i in 1:nrow(excluded_patients)) {
        na_count <- sum(is.na(excluded_patients[i, -1]))
        total_cols <- ncol(excluded_patients) - 1
        cat(sprintf("    %s: %d/%d NA (%.1f%%)\n", 
                    excluded_patients$subject_id[i],
                    na_count, total_cols,
                    na_count/total_cols*100))
      }
    }
    
    strategy_used <- "complete_cases_strict"
    
  } else {
    # 其他时间窗口：使用默认策略
    cat(sprintf("窗口 %s：采用默认策略\n", window_name))
    
    complete_patients <- processed_data %>%
      filter(rowSums(is.na(dplyr::select(., -subject_id))) == 0)
    
    strategy_used <- "default_complete_cases"
  }
  
  # 检查最终患者数量
  cat(sprintf("最终用于聚类的患者数: %d\n", nrow(complete_patients)))
  cat(sprintf("使用策略: %s\n", strategy_used))
  
  # 检查是否有足够患者进行聚类
  if(nrow(complete_patients) < 3) {
    cat(sprintf("⚠️ 警告: %s窗口患者数过少 (%d)，无法进行有效聚类\n", 
                window_name, nrow(complete_patients)))
    return(NULL)
  }
  
  if(nrow(complete_patients) < 5) {
    cat(sprintf("⚠️ 注意: %s窗口患者数较少 (%d)，聚类结果可能不稳定\n", 
                window_name, nrow(complete_patients)))
  }
  
  # ================== 重要：完全移除插补代码，确保没有NA ==================
  # 最终检查：确保没有NA值
  if(any(is.na(complete_patients[, -1]))) {
    cat("⚠️ 发现剩余NA值，进行最终清理...\n")
    complete_patients <- complete_patients[complete(complete_patients), ]
  }
  
  cat(sprintf("✓ %s窗口数据处理完成: %d患者，0个NA值\n", 
              window_name, nrow(complete_patients)))
  ...
  numeric_cols <- names(complete_patients)[-1]
  for(col in numeric_cols) {
    if(is.numeric(complete_patients[[col]])) {
...
# ================== 在执行聚类前添加全局缺失值分析 ==================
cat("开始全局缺失值分析...\n")
global_missing_analysis <- analyze_missing_data_global(ppv_data, key_metrics, time_windows)

# 保存缺失值分析报告
write.csv(global_missing_analysis, "global_missing_data_analysis.csv", row.names = FALSE)
...
standardize_cluster_patterns <- function(window_memberships) {
  ...
  for(window_name in names(window_memberships)) {
    ...
    if(length(cv_col) > 0 && length(steps_col) > 0) {
      ...
      cluster_features <- cluster_features %>%
        mutate(
          pattern_type = case_when(
            cv_rhr_z < -0.5 & steps_max_z > 0.5 ~ "Good_Pattern",      # 低CV，高Steps = 良好模式
            cv_rhr_z > 0.5 & steps_max_z < -0.5 ~ "Poor_Pattern",      # 高CV，低Steps = 较差模式  
            cv_rhr_z < 0 & steps_max_z < 0 ~ "Low_Activity",            # 低CV，低Steps = 低活动
            cv_rhr_z > 0 & steps_max_z > 0 ~ "High_Variability",       # 高CV，高Steps = 高变异
      ...
      print(cluster_features %>% dplyr::select(max_cluster, pattern_type, unified_color, cv_rhr_z, steps_max_z))
...
create_cross_window_pattern_comparison <- function(cluster_patterns, key_metrics) {
  ...
  all_patterns <- bind_rows(cluster_patterns) %>%
    dplyr::select(window, max_cluster, pattern_type, unified_color, cv_rhr_z, steps_max_z)
  ...
  # 创建特征空间图 (CV vs Steps)
  p_feature_space <- ggplot(all_patterns, aes(x = cv_rhr_z, y = steps_max_z)) +
