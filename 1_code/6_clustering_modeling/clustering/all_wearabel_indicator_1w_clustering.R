...
####read data
ppv_data <- read.csv("3_data_analysis/6_clustering_modeling/data_prepare/1w/mfuzz_D_Surg1_8h_filtered.csv", check.names = FALSE)
cat_data <- read.csv("3_data_analysis/6_clustering_modeling/data_prepare/1w/mfuzz_NoD_Surg0_8h_filtered.csv", check.names = FALSE)

str(ppv_data)
str(cat_data)

dir.create("3_data_analysis/6_clustering_modeling/mfuzz/multi_metrics/1w", recursive = TRUE, showWarnings = FALSE)
setwd("3_data_analysis/6_clustering_modeling/mfuzz/multi_metrics/1w")
...
# -------------------- 2. 提取多指标数据 --------------------
# 函数：提取每个选定指标的术前-4到术后7天的数据
extract_periop_data <- function(data, metric) {
  cols <- c()
  for (day in c(-4:-1, 0:7)) {
    if (day < 0) {
...
#   mutate(
#     max_cluster = ppv_main_clusters,
#     # 这里的 membership 是 mfuzz 的结果
#     cluster_membership = apply(ppv_clustering$membership, 1, function(row) paste(round(row, 2), collapse=","))
#   )
# 
# cat_results <- cat_multi %>%
#   mutate(
#     max_cluster = cat_main_clusters,
#     cluster_membership = apply(cat_clustering$membership, 1, function(row) paste(round(row, 2), collapse=","))
#   )
...
pca_cluster_plot <- ggplot(pca_plot_data, aes(x = PC1, y = PC2, color = Cluster)) +
  ...
  labs(title = "PPV clustering group",
