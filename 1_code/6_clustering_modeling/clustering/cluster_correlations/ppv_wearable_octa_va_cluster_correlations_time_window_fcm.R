...
# ===============================================
# 修复ID列识别错误
# 扩展standardize_id_column函数以识别更多ID列名
# ===============================================

# 改进的ID列标准化函数（针对你的数据）
standardize_id_column <- function(data) {
  cat("检查数据列名:", paste(names(data), collapse = ", "), "\n")
  
  # 扩展ID列的可能名称（按优先级排序）
  possible_id_columns <- c("subject_id", "Patient_ID", "ID", "patient_id", "id", "Subject_ID")
  
  # 检查哪个ID列存在
  existing_id_col <- NULL
  for(col_name in possible_id_columns) {
    if(col_name %in% names(data)) {
      existing_id_col <- col_name
      cat(sprintf("找到ID列: '%s'\n", col_name))
      break
    }
  }
  
  if(is.null(existing_id_col)) {
    cat("错误：找不到ID列。可用列名:", paste(names(data), collapse = ", "), "\n")
    stop("找不到ID列。请检查数据中是否包含患者ID列。")
  }
  
  # 如果不是subject_id，则重命名
  if(existing_id_col != "subject_id") {
    cat(sprintf("将 '%s' 重命名为 'subject_id'\n", existing_id_col))
    names(data)[names(data) == existing_id_col] <- "subject_id"
  } else {
    cat("ID列已经是 'subject_id'\n")
  }
...
# 检查wearable_data是否存在且不为空
if(exists("wearable_data") && length(wearable_data) > 0) {
  cat("标准化可穿戴设备数据ID列:\n")
  for(i in seq_along(wearable_data)) {
    cat(sprintf("处理 %s 数据...\n", names(wearable_data)[i]))
    wearable_data[[i]] <- standardize_id_column(wearable_data[[i]])
  }
} else {
  cat("⚠️ wearable_data 不存在或为空\n")
}

# 标准化outcome_data
if(exists("outcome_data") && !is.null(outcome_data)) {
  cat("\n标准化OCTA预后数据ID列:\n")
  outcome_data <- standardize_id_column(outcome_data)
  cat("OCTA数据ID标准化完成\n")
} else {
  cat("⚠️ outcome_data 不存在\n")
}
...
if(exists("wearable_data") && length(wearable_data) > 0) {
  cat("可穿戴设备数据验证:\n")
  for(i in seq_along(wearable_data)) {
    window_name <- names(wearable_data)[i]
    data <- wearable_data[[i]]
    cat(sprintf("- %s: %d 行, subject_id列存在: %s\n", 
                window_name, nrow(data), "subject_id" %in% names(data)))
    if("subject_id" %in% names(data)) {
      cat(sprintf("  前3个ID: %s\n", paste(head(data$subject_id, 3), collapse = ", ")))
    }
  }
}
...
if(exists("outcome_data") && !is.null(outcome_data)) {
  cat("\nOCTA预后数据验证:\n")
  cat(sprintf("- 行数: %d, subject_id列存在: %s\n", 
              nrow(outcome_data), "subject_id" %in% names(outcome_data)))
  if("subject_id" %in% names(outcome_data)) {
    cat(sprintf("  前3个ID: %s\n", paste(head(outcome_data$subject_id, 3), collapse = ", ")))
  }
}
...
# 检查 baseline 数据的详细结构
if(exists("wearable_data") && "baseline" %in% names(wearable_data)) {
  cat("baseline 数据详细检查:\n")
  ...
  cat("Hard_Cluster 列是否存在:", "Hard_Cluster" %in% names(baseline_data), "\n")
...
if(exists("outcome_data") && !is.null(outcome_data)) {
  ...
  cat("Hard_Cluster 列是否存在:", "Hard_Cluster" %in% names(outcome_data), "\n\n")
}
...
# 标准化聚类列名的函数（针对你的数据）
standardize_cluster_columns <- function(data, data_name) {
  cat(sprintf("标准化 %s 的聚类列名...\n", data_name))
  
  # 检查可能的聚类列名（按优先级排序）
  possible_cluster_cols <- c("max_cluster", "Hard_Cluster", "cluster", "Cluster")
  possible_membership_cols <- c("max_membership", "Max_Membership", "membership", "Membership")
  
  # 找到聚类列
  existing_cluster_col <- NULL
  for(col_name in possible_cluster_cols) {
    if(col_name %in% names(data)) {
      existing_cluster_col <- col_name
      cat(sprintf("  找到聚类列: '%s'\n", col_name))
      break
    }
  }
  ...
  # 找到隶属度列
  existing_membership_col <- NULL
  for(col_name in possible_membership_cols) {
    if(col_name %in% names(data)) {
      existing_membership_col <- col_name
      cat(sprintf("  找到隶属度列: '%s'\n", col_name))
      break
    }
  }
  
  # 重命名聚类列
  if(!is.null(existing_cluster_col) && existing_cluster_col != "max_cluster") {
    cat(sprintf("  将 '%s' 重命名为 'max_cluster'\n", existing_cluster_col))
    names(data)[names(data) == existing_cluster_col] <- "max_cluster"
  }
  
  # 重命名隶属度列
  if(!is.null(existing_membership_col) && existing_membership_col != "max_membership") {
    cat(sprintf("  将 '%s' 重命名为 'max_membership'\n", existing_membership_col))
    names(data)[names(data) == existing_membership_col] <- "max_membership"
  }
  
  # 检查最终结果
  has_cluster <- "max_cluster" %in% names(data)
  has_membership <- "max_membership" %in% names(data)
  
  cat(sprintf("  结果: max_cluster存在=%s, max_membership存在=%s\n", has_cluster, has_membership))
...
if(exists("wearable_data") && length(wearable_data) > 0) {
  for(i in seq_along(wearable_data)) {
    window_name <- names(wearable_data)[i]
    wearable_data[[i]] <- standardize_cluster_columns(wearable_data[[i]], paste("wearable", window_name))
  }
}

# 标准化预后数据的聚类列
if(exists("outcome_data") && !is.null(outcome_data)) {
  outcome_data <- standardize_cluster_columns(outcome_data, "outcome")
}
...
check_data_completeness <- function() {
  ...
  if(exists("wearable_data") && length(wearable_data) > 0) {
    for(window_name in names(wearable_data)) {
      data <- wearable_data[[window_name]]
      has_id <- "subject_id" %in% names(data)
      has_cluster <- "max_cluster" %in% names(data)
      has_membership <- "max_membership" %in% names(data)
      
      cat(sprintf("%s: ID=%s, 聚类=%s, 隶属度=%s\n", 
                  window_name, has_id, has_cluster, has_membership))
      
      if(!has_id || !has_cluster) {
        all_good <- FALSE
      }
    }
  }
  ...
  # 检查outcome_data
  if(exists("outcome_data") && !is.null(outcome_data)) {
    has_id <- "subject_id" %in% names(outcome_data)
    has_cluster <- "max_cluster" %in% names(outcome_data)
    has_membership <- "max_membership" %in% names(outcome_data)
    
    cat(sprintf("outcome: ID=%s, 聚类=%s, 隶属度=%s\n", 
                has_id, has_cluster, has_membership))
...
if(data_complete) {
  cat("✅ 所有数据列名标准化完成，可以继续分析\n")
} else {
  cat("❌ 还有数据列名问题，需要进一步检查\n")
}
