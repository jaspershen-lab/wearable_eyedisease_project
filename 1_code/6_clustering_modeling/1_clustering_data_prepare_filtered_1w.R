...
min_required_days <- round(length(seq(-4, 7)) * 0.7)
...
# 查看筛选后每个时间点的数据完整度
day_completeness <- data.frame(day_point = seq(-4, 7))
for(day in seq(-4, 7)) {
  day_cols <- day_columns[[as.character(day)]]
  if(length(day_cols) > 0) {
...
# 创建输出目录
output_dir <- "3_data_analysis/6_clustering_modeling/data_prepare/1w"
...
participant_completeness <- valid_subject_days %>%
  ...
  mutate(total_days = length(seq(-4, 7)),
