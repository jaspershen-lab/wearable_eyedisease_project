...
create_baseline_characteristics_heatmap <- function(data, stats_results) {
  ...
  p_heatmap <- ggplot(all_means, aes(x = Parameter_Clean, y = as.factor(max_cluster), fill = Mean_Value)) +
    ...
    labs(
      title = "Pre-operative Baseline Characteristics Comparison",
      subtitle = paste("T0 OCTA (0_21 Wide-field) + Pre-Vision by Outcome Clusters | n =", nrow(data)),
      x = "",
      y = "Outcome Cluster",
  ...
  # 保存图片
  ggsave("plots/baseline_characteristics_heatmap.pdf", p_heatmap, 
         width = 16, height = 10, device = "pdf")
  ggsave("plots/baseline_characteristics_heatmap.png", p_heatmap, 
         width = 16, height = 10, dpi = 300)
  
  cat("✓ 基线特征热图已保存\n")
  
  # 创建简化版热图（只显示主要参数）- 按照你的风格
  if(nrow(stats_results) > 0) {
    # 获取显著差异的参数
      ...
      if(nrow(significant_data) > 0) {
        p_heatmap_sig <- ggplot(significant_data, aes(x = Parameter_Clean, y = as.factor(max_cluster), fill = Mean_Value)) +
          ...
          labs(
            ...
            subtitle = "Only parameters with significant baseline differences between clusters",
            x = "Baseline Parameters (0_21 Region + Vision)",
            y = "Outcome Cluster"
          )
        
        ggsave("plots/baseline_significant_heatmap.pdf", p_heatmap_sig, 
               width = 12, height = 8)
        ggsave("plots/baseline_significant_heatmap.png", p_heatmap_sig, 
...
# 在你的主代码中，在统计分析（baseline_stats）完成后调用：

# 首先确保已经有 baseline_stats
...
if(nrow(baseline_comprehensive) > 0 && exists("baseline_stats")) {
  baseline_heatmap_results <- create_baseline_characteristics_heatmap(baseline_comprehensive, baseline_stats)
  if(!is.null(baseline_heatmap_results)) {
    cat("✓ 基线特征分组热图生成完成\n")
  }
} else {
